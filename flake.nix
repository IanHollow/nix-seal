{
  ...
  outputs = inputs: let
    ...
    in
      pkgs.rustPlatform.buildRustPackage {
        ...
        cargoBuildFlags = ["--package" "nix-seal"];
  ...
  in {
    ...
    apps = eachSystem (system: {
      default = {type = "app"; program = "${packageFor system}/bin/nix-seal";};
      nix-seal = {type = "app"; program = "${packageFor system}/bin/nix-seal";};
    });
    devShells = eachSystem (system: let
      pkgs = inputs.nixpkgs.legacyPackages.${system};
    in {
      default = pkgs.mkShell {
        packages = with pkgs; [cargo rustc rustfmt clippy cargo-deny cargo-audit nixfmt-rfc-style] ++ lib.optionals stdenv.hostPlatform.isDarwin [libiconv];
      };
    });
    checks = eachSystem (system: {inherit (inputs.self.packages.${system}) nix-seal;});
