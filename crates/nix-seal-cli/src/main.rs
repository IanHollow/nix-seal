...
/// Schema identifier for the generated, public deployment lock.
const PUBLIC_LOCK_SCHEMA: &str = "nix-seal.public-lock.v1";
const PUBLIC_LOCK_MAX_BYTES: u64 = 2 * 1024 * 1024;

/// The checked-in bridge between a public policy plan and target-local
/// ciphertext artifacts. It is deliberately generated by `provision`: cache
/// addresses and source hashes must never be copied from command output.
#[derive(Clone, serde::Deserialize, serde::Serialize)]
#[serde(deny_unknown_fields, rename_all = "camelCase")]
struct PublicLockV1 {
    schema: String,
    /// Public plan identities, keyed by their stable plan IDs. Keeping the
    /// complete public identity set makes this lock useful for every valid
    /// plan, including plans without a recovery identity.
    identities: BTreeMap<String, String>,
    #[serde(default)]
    artifacts: BTreeMap<String, BTreeMap<String, PublicLockArtifactV1>>,
}

#[derive(Clone, serde::Deserialize, serde::Serialize)]
#[serde(deny_unknown_fields, rename_all = "camelCase")]
struct PublicLockArtifactV1 {
    cache_key: String,
...
struct ProvisionArgs {
    ...
    #[arg(long)]
    cache_root: Option<PathBuf>,
    /// Generated public deployment lock to create or update after successful
    /// provisioning. This file contains only public recipients and artifact
    /// addresses; it never contains an identity or plaintext.
    #[arg(long)]
...
fn new_public_lock(plan: &nix_seal_core::PlanV1) -> PublicLockV1 {
    PublicLockV1 {
        schema: PUBLIC_LOCK_SCHEMA.to_owned(),
        identities: plan
            .identities
            .iter()
            .map(|(id, identity)| (id.as_str().to_owned(), identity.public.clone()))
            .collect(),
        artifacts: BTreeMap::new(),
    }
}

fn read_public_lock(path: &Path) -> Result<PublicLockV1> {
    let metadata = fs::symlink_metadata(path)
        .with_context(|| format!("could not inspect public lock {}", path.display()))?;
    if !metadata.file_type().is_file() || metadata.file_type().is_symlink() {
        bail!("public lock must be a non-symlink regular file");
    }
    if metadata.len() > PUBLIC_LOCK_MAX_BYTES {
        bail!("public lock exceeds the 2 MiB safety limit");
    }
    let mut bytes = Vec::with_capacity(usize::try_from(metadata.len()).unwrap_or(0));
    fs::File::open(path)
        .with_context(|| format!("could not open public lock {}", path.display()))?
        .take(PUBLIC_LOCK_MAX_BYTES + 1)
        .read_to_end(&mut bytes)
        .context("could not read public lock")?;
    if bytes.len() as u64 > PUBLIC_LOCK_MAX_BYTES {
        bail!("public lock exceeds the 2 MiB safety limit");
    }
    let lock: PublicLockV1 =
        serde_json::from_slice(&bytes).context("public lock is not valid nix-seal JSON")?;
    if lock.schema != PUBLIC_LOCK_SCHEMA {
        bail!("public lock has an unsupported schema");
    }
    Ok(lock)
}

fn write_public_lock_atomic(path: &Path, lock: &PublicLockV1) -> Result<()> {
    if path.extension().is_none_or(|extension| extension != "json") {
        bail!("public lock path must use a .json extension");
    }
    let parent = path.parent().context("public lock path has no parent")?;
    let parent_metadata = fs::symlink_metadata(parent)
        .with_context(|| format!("public lock parent {} does not exist", parent.display()))?;
    if !parent_metadata.file_type().is_dir() || parent_metadata.file_type().is_symlink() {
        bail!("public lock parent must be an existing non-symlink directory");
    }
    if path.exists() {
        let metadata = fs::symlink_metadata(path)?;
        if !metadata.file_type().is_file() || metadata.file_type().is_symlink() {
            bail!("refusing to replace a non-regular public lock");
        }
    }
    let mut bytes = serde_json::to_vec_pretty(lock).context("could not encode public lock")?;
    bytes.push(b'\n');
    let mut staged = tempfile::NamedTempFile::new_in(parent)
        .context("could not stage public lock transaction")?;
    #[cfg(unix)]
    staged
        .as_file_mut()
    ...
    staged
        ...
        .context("could not durably stage public lock")?;
    staged
        .persist(path)
        .map_err(|error| error.error)
        .context("could not atomically publish public lock")?;
    fs::File::open(parent)
        .and_then(|directory| directory.sync_all())
        .context("public lock changed but directory durability is unknown")?;
    Ok(())
}

fn update_public_lock(
    path: &Path,
    plan: &nix_seal_core::PlanV1,
    target: &nix_seal_core::Id,
    generation: u64,
    artifacts: &[(nix_seal_core::Id, &'static str, nix_seal_rekey::RekeyResult)],
) -> Result<()> {
    let expected = new_public_lock(plan);
    let mut lock = if path.exists() {
        read_public_lock(path)?
    } else {
        expected.clone()
    };
    if lock.identities != expected.identities {
        bail!("public lock identities do not match the current plan; refusing to overwrite it");
    }
    let target_artifacts = artifacts
        .iter()
        .map(|(secret, _, result)| {
            (
                secret.as_str().to_owned(),
                PublicLockArtifactV1 {
                    cache_key: result.cache_key.clone(),
                    source_ciphertext_hash: result.source_ciphertext_hash.clone(),
                    artifact_generation: generation,
                },
            )
        })
        .collect();
    lock.artifacts
        .insert(target.as_str().to_owned(), target_artifacts);
    write_public_lock_atomic(path, &lock)
}
...
fn run_provision(arguments: ProvisionArgs, json: bool) -> Result<()> {
    ...
    if let Some(lock_file) = arguments.lock_file.as_deref() {
        update_public_lock(
            lock_file,
            &plan,
            &arguments.target,
...
mod tests {
    ...
    #[test]
    fn public_lock_is_generated_atomically_from_public_plan_metadata()
    -> Result<(), Box<dyn std::error::Error>> {
        let temporary = tempfile::tempdir()?;
        let path = temporary.path().join("nix-seal.lock.json");
        let mut plan = nix_seal_core::PlanV1::default();
        plan.identities.insert(
            nix_seal_core::Id::parse("admin")?,
        ...
        plan.identities.insert(
            nix_seal_core::Id::parse("recovery")?,
            nix_seal_core::Identity {
                kind: nix_seal_core::IdentityKind::Recovery,
                public: "age1recovery".to_owned(),
            },
        );
        plan.identities.insert(
            ...
            nix_seal_core::Identity {
                ...
                public: "nix-seal-ed25519-v1:public".to_owned(),
            },
        );
        let target = nix_seal_core::Id::parse("host/example")?;
        let secret = nix_seal_core::Id::parse("services/example/token")?;
        let artifacts = vec![(
            secret.clone(),
            "rekeyed",
            nix_seal_rekey::RekeyResult {
                cache_key: "a".repeat(64),
                source_ciphertext_hash: "b".repeat(64),
                artifact_ciphertext_hash: "c".repeat(64),
                recipient_fingerprint: "recipient".to_owned(),
                ciphertext_path: temporary.path().join("ciphertext.age"),
                reused: false,
            },
        )];

        update_public_lock(&path, &plan, &target, 7, &artifacts)?;
        let lock = read_public_lock(&path)?;
        assert_eq!(lock.schema, PUBLIC_LOCK_SCHEMA);
        assert_eq!(lock.identities["admin"], "age1administrator");
        assert_eq!(lock.identities["recovery"], "age1recovery");
        assert_eq!(lock.identities["release"], "nix-seal-ed25519-v1:public");
        assert_eq!(
            lock.artifacts[target.as_str()][secret.as_str()].artifact_generation,
            7
        );
        Ok(())
    }
